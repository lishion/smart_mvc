...
smart mvc(以下简称smart) 是一个使用java开发的简单易用的mvc框架。具有数据自动绑定，IOC，AOP等特性。基于COC(依赖优先配置)思想设计，零配置文件，极速启动。

### 启动项目

由于smart内置了jetty，所以你可以抛弃传统的web项目启动方式，将smart构建的web项目当作一个普通的java应用项目来使用。例如，要启动一个smart构建的项目可以使用以下方法:

```java
public class ServerEntry {
    public static void main(String args[]) throws Exception {
        // 启动项目时需要填入三个参数
      	// 参数1 : 访问项目时的根路径，可以任意设置
        // 参数2 : web资源目录，取决于项目结构
      	// 参数3 : 端口号，可填入任意合法端口号
...
值得一提的是，在启动smart的时候，你甚至不需要编写一个配置文件，smart-mvc大部分功能是基于注解和COC的，在后续的介绍中你也能体会到。

*当然，smart作为一个web框架，在启动方式的选择上是自由的，在启动部署的时候可以当作普通的web应用对待。你完全可以使用传统方式启动项目，但smart仍建议你使用上述方式启动。*  
...
如果你需要自己的设置，请实现**SmartConfig**接口并将实现类放入**classpath**。当然其他的位置也可以，但是将类放在**classpath**可以保证smart以最快速度找到配置文件并开始项目的初始化。   
需要注意的一点是，smart并不支持多个配置文件。如果你定义了多个配置，smart会使用找到的第一个配置文件并忽略其他配置文件。
### bean
...
对于 **单例模式** ，smart会在项目启动阶段生成其实例，并对于每一个请求都使用该实例。   
对于 **原型模式**，smart不会再启动阶段对其实例进行缓存，并对每一个请求都生成新的实例。  
smart建议你使用单例模式进行开发，因为这样会省去大量实例生成/回收的时间。同时，使用单例模式进行开发，请尽量保证你设计的类是一个**无状态，即线程安全的类**。因此你不能在控制器，或是业务层使用成员变量。若无法避免，建议你使用**ThreadLocal**进行隔离。

*在smart中，无需对包扫描进行配置，spring会自动扫描有@Bean注解的类*

### 控制器
控制器无疑是一个mvc框架中最重要的部分，控制器主要由**路由，数据绑定，数据渲染**这三部分内容。
#### 路由(请求路径)
smart中使用 **@Route** 对路由进行配置，**@Routh** 可以注解在类和方法上。因此一个方法的完整路由为:   
 >ip+端口+context path + 类路由 + 方法路由   

其中context path在eclipse中默认是项目路径，idea中可以自己配置。若采用文档中推荐的启动方式，则通过第一个参数传入。  

同时，@Route支持对请求方法进行匹配。如下:
...
##### 简单类型绑定
绑定简单类型需要使用 **@Var("name")** 进行注解，其中name表示前端回传的数据的 **key**。例如:
```java
...
```java
    public void file(@FileVar("smart-file") MultipartFile file){
...
```
在接收时，你可以选择使用**数组或List**。同时，保证前端对应的名字相同:
```html
    <input type="file" name="smart-file">
    <input type="file" name="smart-file">
    <input type="file" name="smart-file">
...
如果你采用**Content-type:applicate-json** 的方式从前端返回数据，请使用@JsonVar对参数进行注解，同时该对象也需要使用@Model进行注解。如:

```java
public User test(@JsonVar User user){
        return user;
}
```

*请确保json数据中的key与对象中的属性名字相同，并且该绑定方式暂时无法绑定简单类型* 
...
那么在绑定的过程中，某个类型为String的变量从前端接受到的值为null，smart将会自动将其转化为字符串  "null"。但是smart并不推荐你使用此功能，从某种角度来说，该功能可能导致潜在的错误被忽略。
#### 数据渲染
